...
async function processAIQuery(query: string, data: any[], sessionId?: string) {
  ...
  // PHASE 4 IMPROVEMENT 15: Universal & Campaign-Specific Optimization Recommendations Handler (Priority: CRITICAL)
  // Handle optimization recommendations both universally and for specific campaigns
    ...
    try {
      ...
      if (isCampaignSpecific) {
        ...
        if (analysisData.length === 0) {
          return {
            content: `No data found for campaign "${campaignName}". Available campaigns: ${[...new Set(data.map(item => item.dimensions.campaign))].join(', ')}`,
            data: {
...
This analysis is focused on "${campaignName}" campaign. Consider these insights in the context of your overall marketing strategy and cross-campaign performance.` : ''}`
  ...
  const content = `🎯 **UNIVERSAL OPTIMIZATION RECOMMENDATIONS**
...
### **Universal Audience Strategy:**
1. **Scale Top Audiences**: Focus on audiences with ROAS > 3.0x across all platforms
2. **Exclude Poor Performers**: Remove audiences with ROAS < 1.5x
3. **Create Lookalikes**: Build lookalike audiences from top 20% performers
4. **Cross-Platform Testing**: Test successful audiences across different platforms
...
### **Immediate Actions (Week 1):**
1. Reallocate 40% of budget from ${bottomPlatforms[0]?.platform} to ${topPlatforms[0]?.platform}
2. Pause creatives with ROAS < 2.0x across all platforms
3. Scale top 3 audiences by 30%
...
- **Creative Performance**: Refresh when CTR drops > 20%`
  ...
  return {
    content: "Error generating universal optimization recommendations. Please try a more specific query.",
    data: {
      type: 'error',
      query: query
    }
