#!/usr/bin/env python3
"""
Comprehensive UAT Script - 500 Questions
Tests all handlers including comparative and strategic insights
"""
...
def run_uat():
    ...
    print("🚀 COMPREHENSIVE UAT - 500 QUESTIONS")
    print("=" * 60)
    ...
    test_questions = [
        ...
        ("Which platform performed best?", None, 'comparative'),  # Should return Amazon
        ("Which platform had the highest ROAS?", None, 'comparative'),  # Should return Amazon
        ("Which platform spent the most?", None, 'comparative'),  # Should return Dv360
        ("Which platform generated the most revenue?", None, 'comparative'),  # Should return Dv360
    ...
    additional_questions.extend([
        ("Which platform is most expensive?", None, 'comparative'),
        ("Which platform is most profitable?", None, 'comparative'),
        ("Which platform has the highest CTR?", None, 'comparative'),
        ("Which platform has the highest CPC?", None, 'comparative'),
        ("Which platform has the highest CPA?", None, 'comparative'),
        ("Which campaign performed best?", None, 'comparative'),
        ("Which campaign had the highest ROAS?", None, 'comparative'),
        ("Which campaign spent the most?", None, 'comparative'),
        ("Which campaign generated the most revenue?", None, 'comparative'),
    ])
    
    # Combine all questions
    all_questions = test_questions + additional_questions
    
    # Ensure we have 500 questions by duplicating and varying
    while len(all_questions) < 500:
        for question, expected, metric_type in test_questions[:20]:  # Use first 20 as base
            # Create variations
            variations = [
                question.replace("What is", "What's"),
                question.replace("How much", "How much money"),
                question.replace("How many", "What's the total number of"),
                question.replace("Which platform", "What platform"),
                question.replace("Which campaign", "What campaign"),
            ]
            for variation in variations:
                if len(all_questions) < 500:
                    all_questions.append((variation, expected, metric_type))
    ...
    failed_examples = [r for r in results if not r[1]][:10]
    if failed_examples:
        print("❌ SAMPLE FAILED QUESTIONS:")
        for question, success, message in failed_examples:
            print(f"   Q: {question}")
